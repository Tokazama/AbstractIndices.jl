...
@testset "AbstractIndex" begin
    ...
    @testset "Array interface" begin
        ...
        @test @inferred(axistype(float_offset)) == typeof(2.0:11.0)
        @test @inferred(axiseltype(float_offset)) == Float64
        @test @inferred(indextype(float_offset)) == Base.OneTo{Int}
        @test @inferred(indexeltype(float_offset)) == Int
    end
