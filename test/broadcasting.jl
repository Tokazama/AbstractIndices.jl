@testset "Binary broadcasting operations (.+)" begin
    ...
    @testset "partially named dims" begin
        x = IArray(ones(3, 5), (:x, nothing))
        y = IArray(ones(3, 5), (nothing, :y))
    ...
    @testset "Dimension disagreement" begin
        @test_throws DimensionMismatch .+(
            IArray(zeros(3, 3, 3, 3), (:a, :b, :c, :d)),
            IArray(ones(3, 3, 3, 3), (:w, :x, :y, :z))
        )
    end
    =#
    ...
    @testset "broadcasting" begin
        v = IArray(zeros(3,), (:time,))
        m = IArray(ones(3, 3), (:time, :value))
        s = 0
    ...
    #= TODO figure out how to write this for IArray constructors
    @testset "Mixed array types" begin
        casts = (
            IArray{(:foo, :bar)},  # Named Matrix
            x->IArray{(:foo,)}(x[:, 1]),  # Named Vector
            x->IArray{(:foo, :bar)}(x[:, 1:1]),  # Named Single Column Matrix
            identity, # Matrix
            x->x[:, 1], # Vector
        ...
        for (T1, T2, T3) in Iterators.product(casts, casts, casts)
            ...
            !any(isequal(IArray), (T1, T2, T3, (:foo, :bar))) && continue
