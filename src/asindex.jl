"""
    asindex(keys[, index])

Chooses the most appropriate index type for an keys and index set.

# Examples
```jldoctest keysindex_examples
julia> float_offset = asindex(2.0:11.0)

julia> float_offset[3.0]  # 3.0 is the 2nd position in the keys field, return 2nd position in the index field
2
```

If the input doesn't match the `keys`'s element type then indexing falls back
to default `Int` based indexing.
```jldoctest keysindex_examples
julia> float_offset[3]  # 3 isn't in found in the keys field. return 3rd position in the index field
3
```

If the `keys` field has `Int` elements then indexing with an `Int` will never
bypass the `keys`.
```jldoctest keysindex_examples
julia> int_offset = asindex(2:11, 1:10);

julia> int_offset[3]
2
```

The easiest way around this behavior is just using a `CartesianIndex`
```jldoctest keysindex_examples
julia> int_offset[CartesianIndex(3)]
3
```


It's also possible to use a tuple of `Symbol`s to represent a vector.
```jldoctest
julia> symbol_index = asindex((:one, :two, :three))

julia> symbol_index[:one]
1

julia> symbol_index[:three]
3
```

Note that an `AbstractIndex` acts much like a `NamedTuple`. Iteration produces
the actual values but the use of `pairs` produces the name-value pairs.
"""
asindex(keys::TupOrVec, index::TupOrVec) = AxisIndex(keys, index)

asindex(keys::TupOrVec, ::OneTo) = OneToIndex(keys)

asindex(keys::NTuple{N}) where {N} = asindex(keys, OneTo(N))

asindex(keys::NTuple{N,Symbol}, index::AbstractVector) where {N} = LabelIndex(keys, index)

function asindex(keys::NTuple{N,T}, index::AbstractVector) where {N,T}
    if isbitstype(T)
        LabelIndex(keys, index)
    else
        asindex([keys...])
    end
end

asindex(keys::AbstractVector) = asindex(keys, axes(keys, 1))
