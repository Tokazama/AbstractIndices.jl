...
pop_index(x::Index{name}) where {name} = Index(pop(keys(x)), pop(values(x)))
...
function push_index!(x::Index, items...)
    len = length(x) + length(items)
    set_length!(keys(x), len)
    set_length!(values(x), len)
    return x
end
function push_index!(x::Index, items::Pairs...)
    len = length(x) + length(items)
    set_length!(values(x), len)
    ks = keys(x)
    f = unique_offset(ks)
...
pushfirst_index!(x::Index, items...) = push_index!(x, items...)
function pushfirst_index!(x::Index, items::Pairs...)
    len = length(x) + length(items)
    set_length!(values(x), len)
    ks = keys(x)
    f = unique_offset(ks)
